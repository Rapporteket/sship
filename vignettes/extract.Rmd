...
Content as provided by _sship_ has to be extracted and decrypted at the recipient in order to gain access the actual content. The prerequisite is that the recipient holds the private key assosiated with the public key used for encryption. The recipient may use tools within _sship_ to establish the cleartext content or any other tool that handle the [cryptographic algorithms applied by _sship_](https://rapporteket.github.io/sship/articles/overview.html).
...
dec(tarfile = "data.csv__20200101_081500.tar.gz",
    ...
    target_dir = ".")
```
From the example above the file ```data.csv``` will be written to the current working directory. Further informas

## Extract and decrypt content outside R
Extraction and decryption of content that was encrypted and packed by _sship_ (in R) is perfectly possible. In the above R-function (```dec()```) a few things is taken care of "under the hood" and to aid a better understanding a few more things should be added to the above list of "conventions":
...
- the shipment, _i.e._ the compressed tar archive file, consists of three files that given the current example will be named:
...
In the above example both the ```key``` and ```iv``` files are binary. When using OpenSSL for decryption these have to be presented as strings of hexadecimal digits as can be seen in the inline _hexdump_ conversion in the last command. Methods for conversion from binary to hex may vary between systems.
